...
def group_exact_pairs(dataset_path: str) -> list[tuple[str, str]]:
    ...
    Pairs are returned as a list of tuples of the file paths.'''
    dataset_path = pl.Path(dataset_path)
    all_files = list(dataset_path.glob("*.pdb"))
    N_files_old = len(all_files)
    print(f"Found {N_files_old} files(chains) in {dataset_path.stem}.")
...
def extract_dimer_path_tuples(dataset_path: str) -> list[tuple[str, str]]:
    '''Creates a list of tuples with the dimer paths for the directories containing the pdb files at 
    dataset_path. Note that dataset_path should contain subdirectories with the pdb files.
    The function iterates through the sub-directories and extracts the dimer path tuples.
    The dataset path can not contain any invalid subdirectories!'''
    dataset_path = pl.Path(dataset_path)
    subdirs = [str(p) for p in dataset_path.iterdir() if p.is_dir()]

    dimer_path_tuples = []
    for subdir in subdirs:
        dimer_path_tuples.extend(group_exact_pairs(subdir))

    return dimer_path_tuples
...
def add_to_manifest(manifest_path: str, json_dict: dict, counter: int) -> int:
...
def process_dimer(args: tuple[str, str]) -> dict:
    """Extract the feature dictionary for a multimer in cif_file and save it to a torch .pt file at output_path.
    Uses global worker state (split_mapping, output_path, backbone_only) initialized once per worker."""
    chain_files = args
    
    # Use global worker state instead of receiving as arguments
    global _worker_split_mapping, _worker_output_path, _worker_backbone_only, _worker_pipeline
    split_mapping = _worker_split_mapping
    output_path = _worker_output_path
    backbone_only = _worker_backbone_only
    ...
    folder_name = chain_files[0].parent.name
    for _, chain_dict in feature_dict.items():
        chain_dict["protein_id"] = protein_id

    files_path = output_path / "pt_files" / folder_name
    files_path.mkdir(parents=True, exist_ok=True)

    out_file = files_path / f"{protein_id}.pt"

    try:
        split = split_mapping[protein_id]
    except KeyError:
        return None # Skip this dimer if it is not present in mint_index.json
...
def load_split_mapping(json_path: str) -> dict[str, str]:
    data = json.loads(pl.Path(json_path).read_text())

    # data is a list of dicts; build a mapping id → split
    return {entry["id"]: entry["split"] for entry in data}


# Global variables for worker processes (initialized once per worker)
_worker_split_mapping = None
_worker_output_path = None
_worker_backbone_only = None
_worker_pipeline = None


def _init_worker(split_json_path: str, output_path: str, backbone_only: bool) -> None:
    """Initialize worker process by loading split_mapping once per worker.
    This avoids pickling the large dictionary for every task."""
    global _worker_split_mapping, _worker_output_path, _worker_backbone_only, _worker_pipeline
    _worker_split_mapping = load_split_mapping(split_json_path)
    _worker_output_path = pl.Path(output_path)
    _worker_backbone_only = backbone_only
    _worker_pipeline = build_pipeline(backbone_only)
...
def process_dataset(
    ...
    """Extracts the feature dict for each dimer from all pdb files in the subdirectories of dataset_path dir
       and saves dimer feats. to output_path dir as .pt files.
       - split_json_path: Path to the json file containing the split mapping."""
    ...
    dimer_files = extract_dimer_path_tuples(dataset_path)
    # Only pass dimer pairs to workers - split_mapping, output_path, and backbone_only
    # are loaded once per worker via initializer to avoid pickling overhead
    ...
    with Pool(processes=N_workers, 
              ...
              initargs=(split_json_path, str(output_path), backbone_only)) as pool:
        for out_json in tqdm(pool.imap_unordered(process_dimer, args),
                             desc="Processing AFDB_DDI dataset",
                             total=len(args)):
            dimer_counter = add_to_manifest(manifest_path, out_json, dimer_counter)
    
    print(f"Saved {dimer_counter} dimer .pt files to {output_path}.")
    return None