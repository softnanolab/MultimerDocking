...
def scatterplot_apo(x, y, save_to=None, xlabel=None, ylabel=None, regplot=False):
    ...
    if regplot:
        sns.regplot(
            ...
            y=y,
            color="steelblue",
            scatter_kws={"s": 10, "alpha": 0.8, "edgecolor": "k"},
        )
    else:
        # Create scatter plot
    ...
    if regplot:
        ...
        sns.scatterplot(
            x=x, y=y, color="steelblue", alpha=alpha, edgecolor="k", s=point_size
        )
        # Add reference line
        grid_x = np.linspace(0, 1, 100)
        plt.plot(grid_x, grid_x / 2 + 0.5, color="red", linestyle="--")
        ax.plot(
            grid_x, grid_x / 2 + 0.5, color="red", linestyle="--", label="Reference"
        )
