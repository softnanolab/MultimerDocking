...
def exec_subproc(cmd, timeout: int = 100) -> str:
    ...
    try:
        ...
        out, err, return_code = (
            str(rtn.stdout, "utf-8"),
            str(rtn.stderr, "utf-8"),
            rtn.returncode,
        )
        if return_code != 0:
            logging.error(
                '[ERROR] Execuete failed with command "' + cmd + '", stderr: ' + err
            )
...
def tm_ensemble_at_N(samples, gt1, gt2, N=5):
    ...
    tm_native_val = 0.5 * (
        _tmscore(gt1, gt2) + _tmscore(gt2, gt1)
    )  # Note that TM score is not symmetric
    return {
