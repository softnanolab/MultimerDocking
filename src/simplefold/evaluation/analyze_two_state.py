...
def get_argparser():
    parser = argparse.ArgumentParser(
        description="Evaluate the ensemble of protein structures."
    )
    parser.add_argument(
        "--sample_dir", type=str, required=True, help="Path to the data directory."
    )
    parser.add_argument(
        "--data_dir", type=str, required=True, help="Path to the target directory."
    )
    parser.add_argument(
        "--batch_samples", action="store_true", help="Batch evaluation of samples."
    )
    parser.add_argument("--task", type=str, default="codnas", choices=["apo", "codnas"])
    parser.add_argument("--nsample", type=int, default=5)
    parser.add_argument(
        "--tm_bin", type=str, required=True, help="Path to the TMscore binary."
    )
...
def pdb_to_npy(pdb_path, model_num=0, chain_id=None, seqres=None):
    with warnings.catch_warnings():
        ...
        try:
            ...
            print(f"Error opening PDB file {pdb_path}")
            return
    ...
    try:
        ...
        print(f"Error parsing chain {pdb_path}")
        return
    if not seqres:
        return coords, seq
    ...
    if "-" in alignment.seqB:
        print(f"Alignment gaps {pdb_path}")
        return
...
def analyze(pairs_df, samples_dir, output_dir, targets_dir, TMscore_bin, nsample=5):
    ...
    cnt = 0
    for i, (name1, row) in tqdm.tqdm(
        enumerate(pairs_df.iterrows()), total=len(pairs_df)
    ):
        ...
        tm1 = np.array(tm1)
        tm2 = np.array(tm2)
        tm1max_list.append(tm1.max())
        tm2max_list.append(tm2.max())
        ...
        cnt += 1
        print(
            f"{name1}|{name2} : TMens: {(tm1.max() + tm2.max()) / 2} | Native: {(hline+vline)/2}"
        )
    ...
    scatterplot_apo(
        x=tmpair, y=tm_ens_list, save_to=output_dir / f"ens_{samples_dir.stem}.png"
    )
    scatterplot_apo(
        x=tmpair,
        y=ensvar_list,
        save_to=output_dir / f"var_{samples_dir.stem}.png",
        ylabel="TM diversity",
        regplot=True,
    )

    tmens_mean_median = (
        f"{round(np.mean(tm_ens_list), 3)} / {round(np.median(tm_ens_list), 3)}"
    )
...
def main(
    ...
    correls = {
        name: {
            "pearson": pearsonr(
                rmsd_dict[name][getmask(name)], rmsf_dict[name][getmask(name)]
            )[0],
            "spearman": spearmanr(
                rmsd_dict[name][getmask(name)], rmsf_dict[name][getmask(name)]
            ).correlation,
    ...
    print(
        "RMSD per-target correlation mean/median", f"{rmsd_pt_mean} / {rmsd_pt_median}"
    )
    print("###################")
...
def routine(args=None):
    ...
    split_csv = targets_dir / "metadata.csv"
    ...
    if args.batch_samples:
        ...
        runs = [
            ...
            if f.is_dir() and not f.name.startswith("asset")
        ]
    else:
        runs = [working_dir]
    ...
    pd.DataFrame(run_vs_metric).to_csv(
        output_dir / f"metrics_{args.task}.csv", index=False
    )
